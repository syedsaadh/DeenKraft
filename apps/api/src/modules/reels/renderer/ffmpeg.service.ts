...
export class FfmpegService {
  ...
  private resolveFfmpegPath(): string {
    ...
    if (envPath) {
      ...
      throw new InternalServerErrorException(
        `FFMPEG_PATH is set but not found: ${envPath}`,
      );
    }
    ...
    if (ffmpegStatic) {
      console.warn(
        `[reel-renderer] ffmpeg-static path is missing on disk (${ffmpegStatic}). Falling back to system ffmpeg from PATH.`,
      );
    }
  ...
  async concatenateClips(
    ...
    if (clips.length === 0) {
      throw new InternalServerErrorException('At least one clip is required');
    }
    ...
    await new Promise<void>((resolve, reject) => {
      command
        .on('start', (commandLine: string) => {
          console.log(`[reel-renderer] ffmpeg command: ${commandLine}`);
        })
        .on('end', () => resolve())
          ...
          if (error.code === 'ENOENT') {
            reject(
              new InternalServerErrorException(
                'FFmpeg executable not found. Install ffmpeg-static correctly, set FFMPEG_PATH, or install ffmpeg on system PATH.',
              ),
            );
            return;
          }
